...
module PageDefinitions =
  ...
  let pageTitle = sidebarButtonTitle
  ...
  let makeSidebarButton currentPage buttonPage =
    ...
    let buttonClass = if active then "sidebar-button active" else "sidebar-button"
    ...
    a [ (_class buttonClass); (_href (pageRoute buttonPage)) ] (List.append buttonIcon [ encodedText title ])
    ...
    [
      ...
      body [] [
        div [ _class "content-wrapper" ] [
          ...
          div [ _class "header" ] [ encodedText pageHeader ]
          div [ _class "sidebar"] [
            ul [] (
              sidebarOrder
              |> List.map (fun p ->
