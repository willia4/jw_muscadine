...
let makeItemCard ctx (item: Book.Book) = task {
  let jobj = item |> Book.makeJObjectFromModel |> Some
  let title = item.Title
  let icon = jobj |> Items.readItemImageOrDefault
  let link = jobj |> Items.tryReadSlug |> Option.bind (fun slug -> Items.getLinkToItem Book.documentType slug ctx)
  ...
  let GET_index =
    fun next (ctx: HttpContext) ->
      ...
      task {
        ...
        let pageHtml = FrontendHelpers.layout FrontendHelpers.PageDefinitions.Books content ["frontend/item_cards.scss"] ctx
